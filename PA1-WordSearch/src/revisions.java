...
// take in a text file where first two lines dictate rows and cols. all other
// lines
...
// curRow++;
// if (stringSoFar.length() >= MIN_WORD_LENGTH
// && dictionary.contains(stringSoFar)) {
...
// * Author: Derek Tominaga
// * File: WordSearch.java
// * Description: This program will take in a text file
// * where first two lines dictate rows and cols. all other lines
// * go to dictate what the word grid will look like. The program
// * will then scan the rows and cols for substrings of length 3 or
// * more. If the substring is in the dictionary it will add it to
// */
...
// dictionarySet.add(dictionary.nextLine().toLowerCase());
// }
// Scanner gridDef = new Scanner(new File(args[1]));
// int line = 0;
// int height = Integer.valueOf(gridDef.nextLine());// height of grid
// int width = Integer.valueOf(gridDef.nextLine());// width of grid
// String[][] grid = new String[height][width];
...
// line++;
// }
// ArrayList<String> lToRStrings = new ArrayList<String>();
// ArrayList<String> rToLStrings = new ArrayList<String>();
// for (int y = 0; y < height; y++) {
// for (int x = 0; x < width; x++) {
// int curRow = y;
// int curCol = x;
...
// while (curRow < height && curCol < width) {
// stringSoFar = stringSoFar + grid[curRow][curCol];
// reverseSoFar = reverseSoFar
// + grid[curRow][(width - 1) - curCol];
// // col-1 - curCol to work from back to front
// curCol++;
// if (stringSoFar.length() >= MIN_WORD_LENGTH && dictionarySet
// .contains(stringSoFar.toLowerCase())) {
// lToRStrings.add(stringSoFar.toLowerCase());
// }
// if (reverseSoFar.length() >= MIN_WORD_LENGTH
// && dictionarySet
// .contains(reverseSoFar.toLowerCase())) {
// rToLStrings.add(reverseSoFar.toLowerCase());
// }
// }
// }
// }
// ArrayList<String> topBottom = new ArrayList<String>();
// ArrayList<String> bottomTop = new ArrayList<String>();
// for (int y = 0; y < width; y++) {
// for (int x = 0; x < height; x++) {
// int curRow = x;
// int curCol = y;
...
// stringSoFar = stringSoFar + grid[curRow][curCol];
// reverseSoFar = reverseSoFar
// + grid[(height - 1) - curRow][curCol];
// curRow++;// height -1 - curRow is off set to get bottom to
// // top
// if (stringSoFar.length() >= MIN_WORD_LENGTH && dictionarySet
// .contains(stringSoFar.toLowerCase())) {
// topBottom.add(stringSoFar.toLowerCase());
// }
// if (reverseSoFar.length() >= MIN_WORD_LENGTH
// && dictionarySet
// .contains(reverseSoFar.toLowerCase())) {
// bottomTop.add(reverseSoFar.toLowerCase());
// }
// }
// }
// }
// ArrayList<String> foundWords = new ArrayList<String>();
// foundWords.addAll(lToRStrings);
// foundWords.addAll(rToLStrings);
// foundWords.addAll(topBottom);
// foundWords.addAll(bottomTop);
// for (String word : foundWords) {
// System.out.println(word);
